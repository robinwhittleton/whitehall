class MinisterSorter
  ...
  def cabinet_ministers
    ...
    ministers.sort_by! { |person, roles| [roles.map(&:seniority).min, person.sort_key] }
    ministers.map { |person, roles| [person, roles.sort_by(&:seniority)] }
  end
  ...
  def other_ministers
    ...
    ministers.sort_by! { |person, _| person.sort_key }
    ministers.map { |person, roles| [person, roles.sort_by(&:seniority)] }
  end
